...
xin = numpy.linspace(-0.005*8, 0.005*4, num_cols) #expected shift of centr. wav. -8 nm -4 nm 0 4nm (sweep by number of rows)
...
#assume number of mirrors is held constant for now
...
for iXM in range(1):
	for iYM in range(1):
		...
		grating_taper_length_list = 185000*np.ones(num_cols) #all grating taper lengths are the same
		...
		for iX in range(num_cols):
			...
			for iY in range(y_start, num_rows-off_in_end):
				...
				if param2['hoste'] == 3:  # 3==CaWO4
					...
					if param2['meander'] == True:
						constgrating_airholescale_list = [-10, -10] #change the grating coupler sweep later
				...
				if param2['meander'] is True:
					if iY % 2 == 0:
						...
						print ("hole scale list" + str(hole_scale_list))
						param2['hole_group'] = 1
				...
				param2['array_orig_x'] = chipCenterX + 29e4 * iX  # gives 500 um x offset between center of GC chips, meaning there is 280 um spacing between chips
				param2['array_orig_y'] = chipCenterY + 500e3 * iY #should keep the spacing we have right now in y but without the gaps for stamping
				# if iX==0:
...
gdspy.gds_print('11_by_7_sweep.gds', unit=1.0e-9, precision=1.0e-10)
